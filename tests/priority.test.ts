...
describe('Extreme Router - Priority', () => {
  ...
  it('should give priority to static over dynamic', () => {
    testExtreme.register('/test.ext').handler = 'Static Handler'; // Static
    ...
    expect(testExtreme.match('/test.ext')?.handler).toBe('Static Handler'); // Static
  });
  it('should give priority to prefix group over other dynamic', () => {
    testExtreme.register('/test(.ext|.css)').handler = 'PrefixGroup Handler'; // Prefix group Priority 100
    testExtreme.register('/test(.ext|.css)?'); // Optional prefix group Priority 200
    testExtreme.register('/:param(test.ext|test.css)'); // Group param Priority 300
    testExtreme.register('/:param<test\\.ext>'); // Regex param Priority 400
    testExtreme.register('/:test.ext'); // Extension param Priority 500
    ...
    expect(testExtreme.match('/test.ext')?.handler).toBe('PrefixGroup Handler'); // Prefix group
  });
  it('should give priority to optional prefix group over other dynamic', () => {
    testExtreme.register('/test(.ext|.css)?').handler = 'OptionalPrefixGroup Handler'; // Optional prefix group Priority 200
    testExtreme.register('/:param(test.ext|test.css)'); // Group param Priority 300
    testExtreme.register('/:param<test\\.ext>'); // Regex param Priority 400
    testExtreme.register('/:test.ext'); // Extension param Priority 500
    testExtreme.register('/:optional?'); // Optional param Priority 600
    ...
    expect(testExtreme.match('/test.ext')?.handler).toBe('OptionalPrefixGroup Handler'); // Optional prefix group
  });
  it('should give priority to group param over other dynamic', () => {
    testExtreme.register('/:param(test.ext|test.css)').handler = 'Group param Handler'; // Group param Priority 300
    testExtreme.register('/:param<test\\.ext>'); // Regex param Priority 400
    testExtreme.register('/:test.ext'); // Extension param Priority 500
    testExtreme.register('/:optional?'); // Optional param Priority 600
    testExtreme.register('/:param'); // Param Priority 700
    ...
    expect(testExtreme.match('/test.ext')?.handler).toBe('Group param Handler'); // Group param
  });
  it('should give priority to regex param over other dynamic', () => {
    testExtreme.register('/:param<test\\.ext>').handler = 'Regex Param Handler'; // Regex param Priority 400
    testExtreme.register('/:test.ext'); // Extension param Priority 500
    testExtreme.register('/:optional?'); // Optional param Priority 600
    testExtreme.register('/:param'); // Param Priority 700
    testExtreme.register('/*'); // Wildcard Priority 800
    ...
    expect(testExtreme.match('/test.ext')?.handler).toBe('Regex Param Handler'); // Regex param
  });
  it('should give priority to extension param over other dynamic', () => {
    testExtreme.register('/:test.ext').handler = 'Extension Param Handler'; // Extension param Priority 500
    testExtreme.register('/:optional?'); // Optional param Priority 600
    testExtreme.register('/:param'); // Param Priority 700
    testExtreme.register('/*'); // Wildcard Priority 800

    expect(testExtreme.match('/test.ext')).not.toBeNull(); // Extension param
    expect(testExtreme.match('/test.ext')?.handler).toBe('Extension Param Handler'); // Extension param
  });
  it('should give priority to optional param over other dynamic', () => {
    testExtreme.register('/:optional?').handler = 'Optional Param Handler'; // Optional param Priority 600
    testExtreme.register('/:param'); // Param Priority 700
    testExtreme.register('/*'); // Wildcard Priority 800

    expect(testExtreme.match('/test.ext')).not.toBeNull(); // Optional param
    expect(testExtreme.match('/test.ext')?.handler).toBe('Optional Param Handler'); // Optional param
  });
  it('should give priority to param over other dynamic', () => {
    testExtreme.register('/:param').handler = 'Param Handler'; // Param Priority 700
    testExtreme.register('/*'); // Wildcard Priority 800

    expect(testExtreme.match('/test.ext')).not.toBeNull(); // Param
    expect(testExtreme.match('/test.ext')?.handler).toBe('Param Handler'); // Param
  });
});
